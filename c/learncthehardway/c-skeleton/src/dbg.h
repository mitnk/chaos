...
typedef struct {char str[24];} mitnk_time_str;

static inline mitnk_time_str
mitnk_curtime()
{
    time_t ltime; /* calendar time */
    ltime=time(NULL); /* get current cal time */
    struct tm *tm = localtime(&ltime);
    mitnk_time_str ctime_str;
    sprintf(
        ctime_str.str,
...
#define debug(M, ...)
#else
#define debug(M, ...) fprintf(stdout, "[%s] DEBUG %s:%d:%s " M "\n", mitnk_curtime().str, __FILE__, __LINE__, __FUNCTION__, ##__VA_ARGS__)
#endif

#define clean_errno() (errno == 0 ? "None" : strerror(errno))

#define log_err(M, ...) fprintf(stderr, "[%s][ERROR] (%s:%d:%s errno: %s) " M "\n", mitnk_curtime().str, __FILE__, __LINE__, __FUNCTION__, clean_errno(), ##__VA_ARGS__)
